...
def build_yearly_source_disposition_chart_data(rows) -> dict:
    ...
    # EIA represents interstate exports as negative net trade values
    # total imports = interstate_imports + international_imports
    # total Exports = - 1 * interstate_exports + international_exports
    interstate_imports = []
    interstate_exports = []
    ...
    # Combine interstate and international into single import/export totals
    # If both sources are None, preserve None so charts show a gap instead of a false zero
    total_imports = []
    total_exports = []
...
def build_state_comparison_chart_data(rows, year: int) -> dict:
    ...
    # Sort states by generation descending for the generation bar chart
    ...
    # Calculate total imports and exports per state
    # EIA represents exports as negative interstate net trade values
    imports_by_state = []
    exports_by_state = []
    ...
    # Sort import and export lists independently by value descending for the bar charts
    imports_rows = sorted(imports_by_state, key=lambda row: row["total_imports"], reverse=True)
    exports_rows = sorted(exports_by_state, key=lambda row: row["total_exports"], reverse=True)
